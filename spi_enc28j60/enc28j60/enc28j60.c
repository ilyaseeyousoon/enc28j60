...
#define enc28j60_reset_high	HAL_GPIO_WritePin(GPIOA,GPIO_PIN_4, GPIO_PIN_SET)
#define enc28j60_reset_low	HAL_GPIO_WritePin(GPIOA,GPIO_PIN_4, GPIO_PIN_RESET)
...
#define enc28j60_select() HAL_GPIO_WritePin(GPIOA,GPIO_PIN_4, GPIO_PIN_RESET) // slave select
...
{
	enc28j60_select();
	enc28j60_tx(ENC28J60_SPI_RBM); /// много вопросов по передаче по spi!!!
	while(len--)
		*(buf++) = enc28j60_rx();
	enc28j60_release();
}
